 ...
 * ─────────────────────────────────────────────────────────────────────────
 *  POST /api/enquiry   (url-encoded, straight from the dialog)
 *
 *  Vercel serverless function — a direct port of the retired Netlify function
 *  (netlify/functions/enquiry.mjs, see git history), same gates, same JSON
 *  contract, same email. This function is the enforcement point AND the
 *  delivery mechanism:
 *
 ...
 *
 *  ── WHY EMAIL, NOT A HOSTED FORM SERVICE ────────────────────────────────
 *  Netlify Forms' free tier stopped STORING at 100 submissions/month — past
 *  the cap an enquiry vanished with a success response, the worst possible
 *  failure during an ad campaign. SuperHosting mail is the client's own,
 *  already paid for, no per-message product cap at enquiry volume, and
 *  SPF/DKIM for the domain already point at it. There is also no second,
 *  unverified endpoint a bot could POST around the Turnstile check.
 *
 *  ── FAILS CLOSED ────────────────────────────────────────────────────────
 ...
 *     environment: Production — set ALL of these before the first deploy) ──
 *    TURNSTILE_SECRET_KEY  Cloudflare Turnstile secret
 *    SMTP_HOST             SuperHosting mail server, e.g. serverNN.superhosting.bg
 *                          (cPanel → Email → Connect Devices shows the exact
 *                          hostname). NOT semplodesign.com — after the DNS
 *                          move that name resolves to Vercel, not the mailbox.
 ...
 *
 *  ── FALLBACK ────────────────────────────────────────────────────────────
 *  public/api/enquiry.php is the tested SuperHosting/cPanel equivalent (PHP
 *  mail(), config outside public_html). It ships in dist/ so the static build
 *  stays upload-ready for public_html if the site ever moves back.
 * ─────────────────────────────────────────────────────────────────────────
 */
...
/* Vercel's Node runtime pre-parses the body by content type: url-encoded
 * arrives as a plain object, but be liberal — a raw string (some proxies strip
 * the content type) still parses. Everything downstream reads URLSearchParams
 * so the port from the Netlify function stays byte-for-byte comparable. */
...
export default async function handler(req, res) {
  ...
  if ((params.get('bot-field') || '').trim() !== '') {
    return send(res, 200, { ok: true }) // look identical to success, send nothing
  }
  ...
  if (!secret) {
    ...
    return send(res, 503, { error: 'captcha-misconfigured' })
  }

  const token = params.get('cf-turnstile-response') || ''
  if (!token) return send(res, 403, { error: 'captcha', reason: 'missing-token' })
  ...
  let verdict
  try {
    const cfRes = await fetch(SITEVERIFY, {
      method: 'POST',
      headers: { 'content-type': 'application/x-www-form-urlencoded' },
      body: verifyBody,
    })
    verdict = await cfRes.json()
  } catch (e) {
    ...
    return send(res, 502, { error: 'captcha-unreachable' })
  }

  if (!verdict?.success) {
    console.warn('Turnstile rejected a submission:', verdict?.['error-codes'])
    return send(res, 403, { error: 'captcha', reason: verdict?.['error-codes'] })
  }
  ...
  if (!SMTP_HOST || !SMTP_USER || !SMTP_PASS) {
    ...
    return send(res, 503, { error: 'email-misconfigured' })
  }
  ...
  const transporter = nodemailer.createTransport({
    ...
    // a cold function + a slow handshake must not eat the whole duration budget
...
// exported for direct unit-testing of the formatting (not used by Vercel)
export { buildEmail }
